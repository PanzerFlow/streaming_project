...
object StreamingJob extends App with SparkSessionWrapper {  
  ...
  val kafkaReaderConfig = KafkaReaderConfig("b-2.panzerstreaming.crgei7.c25.kafka.us-east-1.amazonaws.com:9092,b-3.panzerstreaming.crgei7.c25.kafka.us-east-1.amazonaws.com:9092,b-1.panzerstreaming.crgei7.c25.kafka.us-east-1.amazonaws.com:9092", "dbserver1.demo.bus_status")
  val schemas  = get_schema("s3://panzerflow-streaming/spark/bus_status.json")
  new StreamingJobExecutor(spark, kafkaReaderConfig, "s3://panzerflow-streaming/checkpoint/job", schemas).execute()
}
    ...
    transformDF.select($"value.payload.after.*")
                ...
                .foreachBatch{
                  (batchDF : DataFrame, _: Long) => {
                    ...
                    .save("s3://panzerflow-streaming/hudi/final")
                  }
                }
                .start()
                .awaitTermination() 
